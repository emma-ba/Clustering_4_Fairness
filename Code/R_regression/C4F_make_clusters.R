...
##### PREP AS CLASSIFICATION or REGRESSION #############
if(problem == 'classif'){
  ...
  data <- read.csv('./compas_clustering_regpb.csv') %>% 
    mutate( decile_score = predicted_score ) %>%
    select(-predicted_score) %>%
    mutate(true_class = floor(true_score/10)) %>%
    select(-true_score) %>%
    as_tibble 
  data %>% glimpse
}
...
make_clust <- function(data, plot_dir='./Results/Viz/', make_viz=F,
                        algo='kmeans', n_clust=7,
                         ...
                         error_type='fnr', iter =  n_clust-1, # c(1:(n_clust-1)),
                         feat_scaling = 'zscore'){
  #### Check arguments ###
  # Error to target
...
##### RUN CLUSTERING EXEC ###############
whole_recap <- run_clustering(n_clust=n_clust, algo=algo, error_type_list=c('accuracy', 'fnr', 'fpr'))
source('./viz_recap.R')
source('./viz_error.R')
source('./viz_distrib.R')
source('./viz_contingency.R')
