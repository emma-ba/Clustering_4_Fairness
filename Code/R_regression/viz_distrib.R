...
basic_feat <- c('juv_fel_count', 'juv_misd_count', 'juv_other_count',
                'priors_count', 'charge_degree', 'decile_score', 'error')
...
for(error_type in error_type_list){
  ...
  res_data_long <- res_data_long %>% 
    mutate(input_clust = factor(input_clust, 
                                levels=c(res_data_long$input_clust %>% unique %>% sort)))
  ...
  p_compas <- res_data_long %>% 
    mutate(pred_compas = decile_score %>% as.factor) %>%
    ggplot(aes(cluster, pred_compas)) + 
    geom_count(alpha=0.4, stroke=0) + 
    scale_radius(range=c(0.2,5)) +
    facet_grid(.~input_clust)  +
    theme_minimal() + 
  ...
  p_all <- p_sex / p_race / p_age / p_priors / p_charge /
    p_juv_fel / p_juv_misd / p_juv_other / p_compas / p_error +
    plot_layout(heights = c(1, 4, 5, 4, 1, 2, 2, 2, 3, 4))
  # p_all %>% print
