...
if(length(error_type_list) == 5){
  hline <- data.frame(error_type, 
                      basic_error = c(basic_acc, basic_fnr, basic_fpr, basic_npr, basic_prec))
} else if(length(error_type_list) == 3){
  hline <- data.frame(error_type, 
                      basic_error = c(basic_acc, basic_fnr, basic_fpr))
}

recap_long <- recap %>% select(-all_of(recap_col_remove)) %>%
  # mutate(error_rate = abs(error_rate/10)) %>%
...
recap_long <- recap_long %>% 
  ...
  mutate(error_rate = as.numeric(error_rate)/10)
recap_long %>% glimpse
...
p %>% 
  ggsave(
    filename = paste0( './Results/Viz/0_', algo, n_clust, '_recap.pdf'),
    device = "pdf",
